...
#                           label = 'end day',
#                           value = Sys.Date()-3)),
        column(6, sliderInput("elev_cdec", "pillow altitude (ft)", min = min(df$elev_cdec, na.rm = TRUE), 
     ...
     # uiOutput("checkbox"),
      fluidRow(
        radioButtons("parameter", "plot parameter",                
                                     ...
                                     selected = "inch")) ,
     #   radioButtons("map_param", "map parameter",                
     #                          choices = unique(df$pname),
     #                          selected = "inch") ),
      

  radioButtons('legend', 'legend', c("on", "off"), selected = "on", inline = T),
...
server <- function(input, output) {
 ...
 # })
  ...
  #  p <- ggplot(df, aes_string(x = "dowy", y = "value")) + geom_point(size = 0.5) + 
  #    geom_line()  + labs(y = NULL, x = NULL) +
  #    scale_y_continuous(sec.axis = dup_axis(name = NULL)) #+ theme_bw(base_size=18) #+
  #     #scale_x_continuous(breaks = dowy, label = date_ch)
  #  
  #  if (input$color != 'none')
